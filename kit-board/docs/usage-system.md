...
An install, an account binding, a recognized sign-in, and a successful provider collection are four different states. The UI can mark a source as receiving data after a coverage-only run; look at the adapter and the actual records. The general `/schedules` page also contains static `connected` flags from `lib/catalog.ts`, not an audit of each external scheduler.
