...
| Codex allowance reader | Embedded rollout readings are built. App-server/web-backend readers are stubs; selecting them does not implement them. Embedded readings still arrive from execution, and `codex_execution` reports the Codex `allowance` capability row, including the `reader_fallback_embedded` state while a stub reader is selected. |
