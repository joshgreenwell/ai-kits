...
Source update, September 17: this checkout implements Cursor local counters and hosted usage/allowance, Codex app-server allowance, Claude OAuth usage, and OpenAI/Anthropic Admin usage/cost (`stubs.rs` is gone). The September 13–15 production receipts and installed-binary labels below are unchanged until a new binary is installed and a real authorized reading is stored.

## Start here

The Observatory is partway through replacing its usage collectors. **V2 local collection works for Claude Code and Codex. V1 local jobs still exist, the Claude browser reader is still v1, and monthly reporting is a separate pipeline with gaps.** Cursor, Codex app-server, Claude OAuth, and Admin API adapters are implemented in this checkout; production receipts for those paths are not yet verified, so USG-027–031 stay Planned. `web_backend` stays unimplemented.
...
| Cursor execution/history/readers | Implemented in this checkout (`cursor_local_state`, `usage_summary`, `dashboard_rpc`). Installed production binaries at the September 15 rebuild still reported them unimplemented. Settings chips follow each build's capability report. Cursor project hooks stay unimplemented. |
...
| Claude allowance reader | Statusline is built and now runs in the `claude_account` adapter, which binds each reading to the account stamped on it when the hook observed it and holds what it cannot attribute. `off` is the only value that stops it; `oauth_usage` also calls the private OAuth usage interface with the existing Claude Code sign-in. Observatory never POSTs a refresh_token. `allowance.claude_oauth_keepalive` (off by default) may spawn Claude Code so *it* refreshes its own store. Statusline stays the documented fallback, and Connections / Allowances say so when OAuth failed. The local deny entry `allowance.claude_reader.statusline` (or a dotted prefix) removes the statusline reader whichever mode the server selects. |
| Codex allowance reader | Embedded rollout readings are built. `app_server` is implemented in this checkout through `codex app-server`; `web_backend` stays unimplemented. Embedded readings still arrive from execution, and `codex_execution` reports the Codex `allowance` capability row, including `reader_fallback_embedded` while `app_server` or `web_backend` is selected. |
| Billing / Admin keys | Anthropic/OpenAI Admin usage and cost adapters are implemented in this checkout from `secrets.json`. No production money receipt from these adapters yet. |
...
The database at `%APPDATA%\Cursor\User\globalStorage\state.vscdb` exists. Discovery reports Cursor present. The existing companion has an enabled Cursor binding and an install override enabling Cursor; its identity is unconfirmed. The last scheduled run at the September 13 audit reported `cursor_execution: prerequisite_missing / not_implemented`, with no records. `cursor_account` was off.

This checkout now reads local counters from that database (`cursor_execution`, parser `+cursor-local1`) and hosted usage/allowance through the Cursor session (`cursor_account`, parser `+cursor-hosted1`). Local counters are not billed and never become hourly buckets. Hosted events are the billed authority. Collection still emits only to a confirmed Cursor identity, so the unconfirmed Windows binding will not receive hosted rows until identity is confirmed. A real authorized hosted reading has not been stored, so USG-027 stays Planned.

The Mac's last report used an older `failed / unrecognized_payload` label for the unfinished adapter. Both binaries report `2.0.0`, so version alone does not prove identical behavior.
