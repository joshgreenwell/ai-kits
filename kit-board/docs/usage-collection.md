...
| Claude OAuth, Codex app-server/web backend | Stub adapters; settings do not make them functional. Selecting the Claude OAuth reader keeps the statusline reader running as the documented fallback and says so. |
...
| Cursor project hooks / live `serve` | Not implemented. |
