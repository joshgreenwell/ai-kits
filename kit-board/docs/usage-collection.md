...
| Claude OAuth, Codex app-server/web backend | Stub adapters; settings do not make them functional and the settings page labels those values `unsupported` from each build's capability report. Selecting the Claude OAuth reader keeps the statusline reader running as the documented fallback and says so. |
| Cursor local state and hosted history | Discovery and binding only; both collectors are stubs, and every Cursor value is labelled `unsupported` until a build reports the mode. |
...
The Claude statusline reader moved out of `claude_execution` into the `claude_account` adapter, which now owns the Claude allowance meter: parser version `2.0.0+statusline1`, adapter `claude_account`, channel `hook_snapshot`, reader `statusline`. Record ids are unchanged, because what derives them — the binding, the channel, and a locator of reader, meter key, and observation time — did not move with the adapter, so a reading the Observatory already holds keeps its id. `allowance.claude_reader` gates that adapter now: `off` is the only value that stops it, and selecting `oauth_usage` (still unimplemented) keeps the statusline reader running as the documented passive fallback while the adapter reports `partial` / `not_implemented`. Codex embedded readings stay in `codex_execution`, because they are a by-product of the same rollout scan.
