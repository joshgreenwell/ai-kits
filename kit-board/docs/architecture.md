...
Companion install keys are random, stored only as hashes in Supabase, and revocable through the authenticated Connections page. Bindings scope records to their assigned accounts. The retired local/browser collector keys are disabled, the site no longer issues their connection files or bundles, and `POST /api/v1/telemetry` returns Gone. Public reset feeds use an allowlist, bounded fetches, a shared refresh lease, and immutable normalized revisions. No collection or calculation invokes an AI model.
