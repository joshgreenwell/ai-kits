...
| `browser/claude-quota/` and unpacked extensions | The same directory is now the v2 browser collector (2.0.0, paired as a browser install) and still carries the v1 upload path as the legacy bridge. | Cut each profile over per [the extension README](../browser/claude-quota/README.md): pause the v1 source, remove the legacy connection on the profile. Remove the v1 path from the extension only after every profile's cutover; the directory stays as the v2 collector. Archive v1 historical data before any removal. |
| `/api/v1/telemetry`, `/api/usage-connections`, legacy browser card | Legacy quota ingress and v1 source management. | Remove after the last browser cutover. A temporary Gone response may make old clients fail clearly. |
...
1. Preserve all v1 browser quota samples; their visibility rules are in place (`allowance_percent_view` keeps them, marks a disabled source `history_only`, and shows a sample the v2 reading duplicates once, as the v2 reading). The replacement is the same extension at version 2.0.0, paired as a browser install ([README](../browser/claude-quota/README.md)); verify it per profile with a real scheduled reading before cutting that profile over.
2. Per profile, once the paired install shows fresh readings for the same account: pause the v1 source on the legacy card, then remove the legacy connection on the extension's options page. Keep the extension: it is now the v2 collector. Remove the v1 upload path from the extension code, `/api/v1/telemetry`, and the legacy card only after every profile's cutover and receipts (USG-026). Do not remove a general Claude browser integration by name confusion.
