...
There is **no implemented bulk v1-to-v2 data migration tool in this checkout**. The companion reparses available logs; it does not import all historical v1 SQLite state. A new migration must provide an explicit source mapping, provenance, repeatable dry run, stable idempotency, and before/after verification. The current allowance schema requires `binding_id`; an administrative import must not fabricate a live installation or use re-enrollment to disguise legacy origin. Extend provenance/import representation if needed.

The existing `allowance_percent_view` hides history from disabled sources/bindings. Change historical read policy before relying on “disabled” as a preservation mechanism, and prevent old and migrated copies from appearing twice. Keep historical samples available to history calculations while ensuring stale samples do not become current allowances. Preserve RLS, security-invoker views, restricted grants, and append-only ledgers.
