...
- Cursor and the provider account/Admin readers are implemented in this checkout. USG-027–031 stay Planned until a real authorized receipt is stored and shown. `web_backend` stays unimplemented.
...
- Envelope v2 and its append-only storage represent independent agent lifecycle, tool invocation/result, project state, pricing dimensions, and configured resource-access evidence. The Claude and Codex local collectors emit those blocks and records under USG-004 through USG-008; Cursor local counters emit requests without tool/agent/project/resource detail; Admin API adapters emit buckets and money, not request lineage; production remains at `buckets_only`.
