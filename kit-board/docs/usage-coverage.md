...
| Spawn depth, nested agents | `spawnDepth` in `agent-<agentId>.meta.json`; a nested agent's file sits under its own parent | `thread_spawn.depth` | SDK messages carry `parent_agent_id`; hooks and OTel events do not | none | none | none | none | `agent.depth`, `agent.parent_key` | Contract and storage ready; collection is USG-005 |
...
| Per-agent token total | sum of the subagent file; the desktop app also writes a task notification with `subagent_tokens`, `tool_uses`, `duration_ms` into the parent transcript | sum of the child thread | none | none | none | none | none | `activity.request.agent.key` | Contract and storage ready; collection is USG-005 |
...
- Neither local format distinguishes a requested model from the resolved model on these request rows, so `model_requested` remains null. Unsupported pricing dimensions remain null, and partial counter objects preserve absent components as null rather than zero.
...
Collection is not built. Envelope v2 and the repository migration can now retain request attribution plus independent spawn/start/resume/finish evidence; the companion still keeps only the older parent relationship.
...
Collection is not built. Envelope v2 and the repository migration now retain independent invocation, result, and resource-access rows; the companion still drops every tool block in the local transcripts. USG-006 and USG-008 implement those collectors.

- Today: legacy `activity.request.tools` and `tool_calls` remain supported, and `tool.event` plus `resource.access` provide stable event identities, outcomes, callers, wrapper joins, multiple results, and configured resource keys. Current companion producers emit neither form. The hook receiver keeps the event name, tool name, and hashed session id but discards `tool_input`, `tool_response`, and `cwd`; no tool hook is installed on the machines we collect from. The Codex scan still omits function/custom-tool call and output lines.
...
- Companion change, Codex: widen the byte filter to `function_call`, `custom_tool_call`, and their outputs; take `command` and `workdir` from `shell_command`, quoted path tokens out of `exec` input, and `*** Update File:` and `*** Add File:` headers out of `apply_patch`; use `turn_context.cwd` only to resolve a relative path from those explicit arguments. The following `token_count` can join execution evidence when the invocation/request identity is supported, but resource access itself receives no invented token allocation. The parity corpus gains tool lines, and the v1 bucket digests must not move.
