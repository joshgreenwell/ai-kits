...
- Superseded 2026-09-21, meter direction: both surfaces above were built counting up, which contradicted the remaining percentage printed directly above the bar. They now read remaining. The summary bar draws `100 - used_percent`, left-anchored, so a fresh window is full and the fill retreats to the right; the chart's Y axis is remaining, the even-pace guide is the diagonal from 100% at phase 0 to 0% at the reset, the dashed allowance boundary moved from `y=100` to `y=0`, and the Y **floor** — not a ceiling — drops in 25-point steps when demand exceeds the allowance. `components/kit/meter.tsx` follows the same rule. The inversion is presentation-only (`meterRemaining`/`projectedRemaining` in `lib/allowance-view.ts`); `lib/telemetry-contract.ts` stays in consumption space, and the "% used" caption, the burn and available-pace rates, the "pts over" overage, and the completed-cycle point totals deliberately do not invert.
