...
- Boundaries preserved: authentication and no-store through the shared route helpers; every read goes through the serialized database queue (`lib/db.ts` now also gates `unsafe`, which the module uses for positional-parameter text, values always bound); bounded output (at most 400 daily or 336 hourly points, grouped rows only); a five-minute bounded cache of at most 96 scopes (including `section=overview|requests|tools|knowledge`); unknown accounts and machines answer 404, invalid ranges 400. Detail ranking discovers keys in the selected range and ranks only those keys, instead of windowing the whole ledger or joining the global resolution views. The overview ranks the hourly ledgers once per read into a `_usage_buckets` temp table in one transaction (the whole months the range touches, plus edge-straddling buckets) and derives the headline, pricing, conversation, straddle, and month-cohort rows from it; the ledger predicates compare `hour` and `bucket_start` against constants so they use the existing indexes. Tokens paints the headline from hourly ledgers first, then fetches request, tool, and knowledge sections in parallel; omitting `section` still returns the full document. Indexes in `20260918090000_usage_query_read_indexes.sql` support the key-then-revision plan.
