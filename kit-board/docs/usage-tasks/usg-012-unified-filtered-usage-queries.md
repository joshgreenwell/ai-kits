...
- Remaining, outside this task: the migration and route deploy with the server build under USG-025; the report-subject crosswalk has no Settings surface yet (map subjects through `PUT /api/usage-report-subjects` until USG-015's follow-up adds one), so production snapshots are listed and not merged until mapped; conversations under the bucket basis are distinct canonical sessions, which USG-017 should label as such; concurrent-read load beyond the queue and cache bounds was not separately soaked.
