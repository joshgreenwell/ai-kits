...
- Boundaries preserved: authentication and no-store through the shared route helpers; every read goes through the serialized database queue (`lib/db.ts` now also gates `unsafe`, which the module uses for positional-parameter text, values always bound); bounded output (at most 400 daily or 336 hourly points, grouped rows only); a five-minute bounded cache of at most 96 scopes (including `section=overview|requests|tools`); unknown accounts and machines answer 404, invalid ranges 400. Tokens fetches those sections in order so the headline can paint from hourly ledgers before request and tool tables run; omitting `section` still returns the full document.
