...
- `lib/usage-pricing.ts` now returns daily per-model cost series using the America/Chicago rate date of the headline rows. Missing dates remain gaps, and every series row retains priced/unpriced tokens, calls, dimensions, rate versions, and cost components. The aggregate estimate and catalog rules are unchanged.
