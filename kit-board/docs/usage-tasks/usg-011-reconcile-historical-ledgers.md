...
- Remaining, outside this task: the Mac host's local checkpoints and the 25 candidate keys need the transcripts, which USG-002's inaccessible-host inventory owns; restoring the private September 13 baseline was verified under USG-002 and not repeated. The migration reached production on September 16 under USG-025, after the server build that reads it, so the deployed view no longer hides history.
