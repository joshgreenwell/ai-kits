...
- [USG-025](usg-025-activate-backfill-and-verify-release.md) — the server build, all schema through `20260919090000`, and request detail at the source are on production; the collector half of criterion 1 was done by the owner on September 18. Open: apply `20260921090000` (collapses the Cursor replay), upgrade the Mac companion to 2.1.0, a scheduled-cycle receipt from each machine on 2.1.0, source-to-screen checks against production, the edge-state pass, and the release record.
...
1. **Schema migrations are applied to production.** Done on September 16 with `supabase db push --linked` for `20260914030000_reconcile_historical_ledgers.sql` (USG-011) and `20260914040000_usage_report_subjects.sql` (USG-012), and on September 18 for `20260918090000_usage_query_read_indexes.sql` (Tokens key-then-revision indexes). `supabase migration list --linked` shows local and remote matching through `20260918090000`. `20260919090000_drop_duplicate_agent_events_index.sql` (drops `agent_events_semantic_time`, a column-for-column duplicate of `agent_events_canonical`) and `20260921090000_request_ledger_revision_uniqueness.sql` (collapses the Cursor `local_db` replay rows to the earliest sighting of each identical content, 383,612 of 392,397 rows on September 21, and renames the request ledger's revision constraint to `activity_requests_revision`) are in the repository and pending production apply; the repository holds seventeen migrations. Disabled-producer history is visible again, monthly snapshots can merge, and mapping a report subject in Settings no longer errors.
2. **Request detail is on at the source, and the first days of it are recorded.** The owner raised the shared settings to version 9 on September 18 (06:03 UTC): `execution.detail_level: "requests_with_tools"`, `project_attribution: "hashed"`, `tool_detail: "hashed_custom"`, both companions enabled for Claude, Codex, and Cursor. Production read on September 21: 472,713 request rows (Claude 14,504; Codex 65,812; Cursor 392,397 rows for only 8,785 requests), 161,671 tool events, 1,259 agent events, 99 project identities under 5 named projects, 0 resource accesses (no knowledge source is configured), 887 allowance readings. The Cursor rows were a replay: the `cursor_execution` reader parsed Cursor's ISO-8601 `createdAt` as an integer, stamped the run clock instead, and re-emitted every bubble each hour with a new content hash, about 130k rows a day, which is what made the request reads take 9 seconds on average (38 seconds worst) and surface as 503s. Companion 2.1.0 (tag `observatory-v2.1.0`) stops that at the source and is installed on this PC; `20260921090000_request_ledger_revision_uniqueness.sql` collapses the 383,612 replay rows to one per request and is the one migration still pending production apply. The read layer now runs under a single 30-second budget and answers an exhausted read with 504 instead of 503.
