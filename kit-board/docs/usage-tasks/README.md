...
- [USG-025](usg-025-activate-backfill-and-verify-release.md) — the server build, all schema through `20260919090000`, and request detail at the source are on production; the collector half of criterion 1 was done by the owner on September 18. Open: upgrade the Mac companion to 2.1.0, a scheduled-cycle receipt from each machine on 2.1.0, source-to-screen checks against production, the edge-state pass, and the release record.
