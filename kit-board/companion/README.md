...
`+appserver1`, `+cursor-local2`, `+cursor-hosted1`, `+admin-usage1`). `web_backend` and the
...
| `observatory projects --apps [--samples]` | How the app projects resolve on this machine, as counts and never a path: each Codex desktop project with its root, thread, and request counts; requests per resolution for Codex (session first), Claude (folder), and Cursor (session); session-against-folder agreement; explicit assignments against the longest root; inherited subagents; forked rollouts; the side records a run would produce. `--samples` adds the titles of a few threads placed only by a root prefix, to check against the Codex sidebar. Reads the app stores and the state; stores nothing. |
| `observatory upgrade-gate --state <copy> --cutoff <rfc3339> [--baseline <copy>]` | Checks a `VACUUM INTO` copy of the state after `run --dry-run --offline` with a new build and exits 3 on any ledger change a release does not allow: a revised `activity.request` or `agent.event`, a new one for history under a semantic key the server never received, a new `tool.event` for history that is not a nested MCP call, a revised `tool.event` other than `PowerShell` or `NotebookRead`, or (with `--baseline`) a tool row that moved to another request. With `--baseline` (a copy taken before the dry run) history is what the previous build had read: a new record at the same instant as one the baseline holds under another key, or at or before the latest activity the baseline holds for its session and agent. Activity the last run had not read yet (written during its scan, or left by a `partial` run) is new, not history. Without `--baseline`, history is anything before `--cutoff`, which fails on that unread activity too; always pass `--baseline`. Prints the counts per type, the changed fields of revised rows, and the pending side records. Reads only. |
...
| `observatory doctor [--offline]` | Effective mode and reason per adapter, prerequisite and credential checks as coverage states, discovery booleans (including Obsidian's registry and vault count), `resources_configured`, `resources_invalid`, `resource_attribution_effective` with its reason (`detail_level`, `denied_locally`, `no_resources`, `ok`), a `claude_statusline` block (below), a `side_records` block (records the Observatory deferred, per type, and the last run's side-record build), v1 schedules found. `--offline` reads the cached config document instead of fetching it, like `run --offline`. Also the `schedule` verdict. Never a token or a path outside the configuration directory. |
...
- `project.catalog`: each project the owner created in the Codex desktop app, read-only from
  `<codex_home>/state_<N>.sqlite` (`projects`, `project_roots`), keyed by the unkeyed
  `sha256(["app-project", app, account_id, project_id])` so a new privacy key never orphans it. A
  project missing from a later successful read becomes a `removed` tombstone (`app_projects_seen`).
- `project.membership`: which app project each ledger session hash and each folder key belongs to.
  Codex sessions follow the app: the thread's own project, an explicit assignment in
  `.codex-global-state.json` (translated through the legacy project-id map), the app's projectless
  list, the root ancestor of a spawned thread, the longest project root of the (worktree-resolved)
  working directory, and otherwise `projectless`. Folders and Cursor composers resolve by the
  longest Codex project root; a worktree resolves to its main repository (`.claude/worktrees`, the
  worktree's `.git` file, or Claude desktop's `git-worktrees.json`) and the answer is cached in
  `member_paths`, so a deleted worktree keeps its project. A store that cannot be read now produces
  none of these records, and the ones already sent stand.
- `name.label`: the readable name beside each hashed tool, MCP namespace, and custom agent name the
  state holds, the role of each Codex subagent session (its `agent_role`, `default` when null,
  `guardian` for guardian reviews), and each Cursor composer (`main` or its subagent type).

`tool_detail` gates labels (`off`: none; `builtin_only`: only agent labels that name a builtin;
`hashed_custom`: all) and `project_attribution = hashed` gates the project records. A record the
server defers is marked locally (`doctor` counts them) and every produced record is sent again once
a week. Local only: paths, folder names, thread titles, and the app store files themselves.

Codex MCP calls made from inside an `exec` script (`item_completed` `McpToolCall`) become
`tool.event` invocations and results linked to the one open `exec` by `parent_invocation_key`, marked
`nested_mcp` locally so they never count toward a request's `tool_calls` or `tools[]`.
