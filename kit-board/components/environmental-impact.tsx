...
/** Suggestions stay apart from the estimate: nothing here changes the footprint the card reports. */
export function EnvironmentalActions({ estimate }: { estimate: EnvironmentalEstimate }) {
  ...
  return (
    <section aria-label="Actions you can take" className="grid gap-4">
...
export function EnvironmentalImpact({ estimate, id, className }: { estimate: EnvironmentalEstimate; id?: string; className?: string }) {
  ...
  return (
    <Card id={id} className={cn('@container/impact scroll-mt-28 gap-0 overflow-hidden py-0', className)} aria-label="Environmental impact">
      ...
      <div className="border-border grid gap-1 border-t px-4 py-3">
        <Disclosure title="Actions you can take">
          <EnvironmentalActions estimate={estimate} />
        </Disclosure>
        <Disclosure title="Scenario factors, scope, methodology, and sources" contentClassName="text-muted-foreground grid gap-4 text-xs leading-relaxed">
          <StatGroup className="border-border text-foreground rounded-[var(--radius-card)] border">
