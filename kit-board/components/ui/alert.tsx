  ...
  "relative grid w-full items-start gap-y-0.5 rounded-lg border px-4 py-3 text-sm grid-cols-[0_1fr] has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        destructive:
          "text-destructive border-destructive/40 bg-destructive/8 *:data-[slot=alert-description]:text-destructive/85",
        warning:
          "text-warning border-warning/40 bg-warning/8 *:data-[slot=alert-description]:text-warning/85",
        success:
          "text-primary border-primary/40 bg-primary/8 *:data-[slot=alert-description]:text-primary/85",
      },
    },
