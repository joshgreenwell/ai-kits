import { feedSources, normalizeFeed, RESET_NORMALIZATION_VERSION, type FeedSource, type ResetDocument } from './reset-feeds';
import { nextResetUrls, normalizeNextReset } from './reset-feed-fallback';
...
type Saved = { version?: string; provenance?: string; current_hash?: string | null; etag?: string | null; last_modified?: string | null };
type Result = { unchanged: true } | { payload: ResetDocument; etag: string | null; last_modified: string | null };

/** One instance per shared-lease sync: at most one request to each fallback endpoint. */
export function createResetFeedFetcher(fetcher: typeof fetch = fetch) {
  ...
  let fallback: Promise<[unknown, unknown]> | undefined;
  const nextReset = () => fallback ??= Promise.all(Object.values(nextResetUrls).map(async url => {
  ...
  return async (source: FeedSource, saved: Saved = {}): Promise<Result> => {
    // Validators belong to one origin AND one normalized schema, never to a fallback.
    const canRevalidate = saved.version === String(RESET_NORMALIZATION_VERSION) && saved.provenance !== 'nextreset' && !!saved.current_hash;
    try {
      ...
      if (source !== 'codex-timeline' && source !== 'codex-announcements') throw error;
      const [archive, status] = await nextReset();
      return { payload: { ...normalizeNextReset(source, archive, status), primary_error: resetFeedFailure(error) }, etag: null, last_modified: null };
    }
  };
}
