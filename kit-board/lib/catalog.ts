...
export const sections: { kind: ReportKind; title: string; path: string; empty: string }[] = [
  ...
  { kind: 'tasks', title: 'Daily tasks', path: '/tasks', empty: 'The next daily briefing and standup will appear here.' },
...
// Standups have no section of their own; they read alongside the briefing for the same day.
export const sectionPath = (kind: ReportKind) => sections.find(section => section.kind === (kind === 'standup' ? 'tasks' : kind))!.path;
